...
async function checkSMSStatus(requestId: string) {
...
// ── STRIPE CHECKOUT ──────────────────────────────────────────────
// Payment is handled via Stripe Checkout (redirect to Stripe's hosted page).
// The secret key lives server-side in the API route — never exposed to the client.
...
const css = (strings: TemplateStringsArray, ...vals: any[]) => strings.reduce((acc, s, i) => acc + s + (vals[i] ?? ""), "");
...
function Chip({ children, color = C.blue, style: sx = {} }: { children: React.ReactNode; color?: string; style?: React.CSSProperties }) {
  return (
    <span style={{
      ...
      fontSize: 11, fontWeight: 800, letterSpacing: 1.5, textTransform: "uppercase" as const,
      ...sx
    }}>{children}</span>
  );
}

function Btn({ children, onClick, variant = "primary", size = "md", style: sx = {}, href, disabled }: { children: React.ReactNode; onClick?: () => void; variant?: string; size?: string; style?: React.CSSProperties; href?: string; disabled?: boolean }) {
  ...
  const base: React.CSSProperties = {
    ...
    fontWeight: 700, letterSpacing: 0.8, textTransform: "uppercase" as const,
...
function Input({ label, ...props }: { label?: string; [key: string]: any }) {
  return (
    <div>
      {label && <label style={{ fontSize: 11, fontWeight: 700, color: C.slate400, letterSpacing: 1.5, textTransform: "uppercase" as const, display: "block", marginBottom: 7 }}>{label}</label>}
      <input {...props} style={{
  ...
  return (
    <div>
      ...
      <textarea {...props} style={{
        ...
        background: "rgba(255,255,255,0.05)", border: `1px solid rgba(255,255,255,0.1)`,
        color: C.white, fontSize: 15, fontFamily: "'Outfit',sans-serif",
        boxSizing: "border-box", resize: "vertical" as const, minHeight: 100, outline: "none",
        ...props.style
      }} />
    </div>
  );
...
function Select({ label, options, ...props }: { label?: string; options: { value: string; label: string }[]; [key: string]: any }) {
  return (
    <div>
      {label && <label style={{ fontSize: 11, fontWeight: 700, color: C.slate400, letterSpacing: 1.5, textTransform: "uppercase" as const, display: "block", marginBottom: 7 }}>{label}</label>}
      <select {...props} style={{
...
function GlassCard({ children, style: sx = {}, glow }: { children: React.ReactNode; style?: React.CSSProperties; glow?: string }) {
  return (
    <div style={{
...
function SectionTitle({ chip, title, sub, light = true, center = true }: { chip?: string; title: string; sub?: string; light?: boolean; center?: boolean }) {
  return (
    <div style={{ textAlign: center ? "center" : "left", marginBottom: 56 }}>
      ...
      {sub && <p style={{ color: light ? C.slate400 : C.slate600, fontSize: 17, lineHeight: 1.7, maxWidth: 560, margin: center ? "0 auto" : undefined }}>{sub}</p>}
    </div>
  );
}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 28px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div onClick={() => setPage("Home")} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 400, fontSize: 10, color: C.blueLt, letterSpacing: 3, textTransform: "uppercase" as const }}>AUTO CARE · SF</div>
          </div>
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
            {TESTIMONIALS.slice(0,2).map(t => (
              <GlassCard key={t.name} style={{ padding:28 }}>
                <Stars n={t.stars} />
                <p style={{ color:"rgba(255,255,255,0.8)", fontSize:14, lineHeight:1.8, margin:"12px 0" }}>&ldquo;{t.text}&rdquo;</p>
            ...
            {PACKAGES.map(pkg => (
              <GlassCard key={pkg.id} glow={selectedPkg === pkg.id ? pkg.glow : undefined}
                ...
                <div style={{ padding:"16px 24px 24px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <Btn onClick={() => setPage("Booking")} variant={selectedPkg===pkg.id ? (pkg.id==="gold"?"gold":"primary") : "ghost"} size="sm" style={{ width:"100%", marginTop:20 }}>
                    Book {pkg.tier}
                  </Btn>
                </div>
              </GlassCard>
...
// Helper: parse Stripe return params from URL on mount
function getStripeReturnState() {
  if (typeof window === "undefined") return { stripeSuccess: false, stripeCancel: false, returnRef: "", sessionId: "", booking: null as any };
  const params = new URLSearchParams(window.location.search);
  const stripeSuccess = params.get("stripe_success") === "true";
  const stripeCancel = params.get("stripe_cancel") === "true";
  const returnRef = params.get("ref") || "";
  const sessionId = params.get("session_id") || "";
  let booking = null;
  const saved = localStorage.getItem("bluebay_booking");
  if (saved) {
    booking = JSON.parse(saved);
    localStorage.removeItem("bluebay_booking");
  }
  if (stripeSuccess || stripeCancel) {
    window.history.replaceState({}, "", "/");
  }
  return { stripeSuccess, stripeCancel, returnRef, sessionId, booking };
}

function BookingPage() {
  const today = new Date();
  const returnState = getStripeReturnState();
  const [month, setMonth] = useState(new Date(today.getFullYear(), today.getMonth(), 1));
  const [selDate, setSelDate] = useState<Date | null>(returnState.booking?.dateISO ? new Date(returnState.booking.dateISO) : null);
  const [selTime, setSelTime] = useState(returnState.booking?.time || "");
  const [selPkg, setSelPkg] = useState(returnState.booking?.selPkg || "");
  const [selVehicle, setSelVehicle] = useState(returnState.booking?.selVehicle || "sedan");
  const [addons, setAddons] = useState<string[]>(returnState.booking?.addons || []);
  const [step, setStep] = useState(returnState.stripeSuccess ? 6 : returnState.stripeCancel ? 5 : 1);
  const [form, setForm] = useState(returnState.booking ? { name: returnState.booking.name, email: returnState.booking.email, phone: returnState.booking.phone, vehicle: returnState.booking.vehicle, address: returnState.booking.address, notes: returnState.booking.notes || "" } : { name:"", email:"", phone:"", vehicle:"", address:"", notes:"" });
  const [loading, setLoading] = useState(false);
  const [ref, setRef] = useState(returnState.returnRef || "");
  const [waiverChecked, setWaiverChecked] = useState(false);
  const [paymentProcessing, setPaymentProcessing] = useState(false);
  const [paymentError, setPaymentError] = useState(returnState.stripeCancel ? "Payment was cancelled. You can try again." : "");
  const [paymentPending, setPaymentPending] = useState(false);

  // Send SMS/email confirmation when returning from Stripe
  useEffect(() => {
    if (returnState.stripeSuccess && returnState.booking) {
      const booking = returnState.booking;
      const smsBody = `Hi ${booking.name}! ✅ Your ${booking.package} detail is confirmed for ${booking.dateStr} at ${booking.time}. Ref: ${returnState.returnRef}. We'll come to ${booking.address}. Payment processed via Stripe. Questions? Call/text (415) 702-8468.`;
      sendSMS(booking.phone, smsBody);
      sendEmail(booking.email, booking.name, `Detail Confirmed — ${booking.dateStr}`, `<h2 style="color:#0EA5E9">Booking Confirmed — BlueBay Auto Care</h2><p><strong>Service:</strong> ${booking.package}</p><p><strong>Vehicle:</strong> ${booking.vehicle} (${booking.vehicleType})</p><p><strong>Date:</strong> ${booking.dateStr} at ${booking.time}</p><p><strong>Address:</strong> ${booking.address}</p><p><strong>Estimated Total:</strong> ${booking.total}</p><p><strong>Ref:</strong> ${returnState.returnRef}</p><p>Payment processed via Stripe. Session: ${returnState.sessionId}</p>`);
    }
  }, [returnState.stripeSuccess, returnState.booking, returnState.returnRef, returnState.sessionId]);
  ...
  const basePrice = pkg ? pkg.prices[selVehicle as keyof typeof pkg.prices] : 0;
  ...
  async function handleStripeCheckout() {
    ...
    // Save booking data to localStorage so we can show confirmation after Stripe redirect
    localStorage.setItem("bluebay_booking", JSON.stringify({
      name: form.name,
      email: form.email,
      phone: form.phone,
      vehicle: form.vehicle,
      vehicleType: VEHICLE_TYPES.find(v=>v.id===selVehicle)?.label,
    ...
    try {
      ...
      const res = await fetch("/api/create-checkout-session", {
        ...
        body: JSON.stringify({
          ...
          addOns: addons.map(a => ({ name: a, price: addonPrices[a] || 0 })),
          amount,
          dateStr,
          time: selTime,
          address: form.address,
      ...
      if (data.url) {
        ...
        setPaymentError("Could not create checkout session. Please try again.");
        setPaymentProcessing(false);
      }
    } catch (err: any) {
    ...
    <div style={{ paddingTop:68, background:C.navy, minHeight:"100vh" }}>
      <div style={{ background: C.navyMid, padding:"60px 28px 80px" }}>
        <div style={{ maxWidth:860, margin:"0 auto" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:28 }}>
                {PACKAGES.map(p => (
                  <div key={p.id} onClick={() => setSelPkg(p.id)} style={{
                    ...
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontWeight:900, color:C.white, fontSize:22 }}>${p.prices[selVehicle as keyof typeof p.prices]}</div>
              ...
              <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:12, padding:20, marginBottom:24 }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom: addons.length > 0 ? 16 : 0 }}>
                  {[
                    ...
                    <div key={k}>
                      <div style={{ fontSize:10, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1, marginBottom:2 }}>{k}</div>
                ...
                {addons.length > 0 && (
                  <div>
                    <div style={{ fontSize:10, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1, marginBottom:6 }}>➕ Add-ons</div>
          ...
          {/* STEP 5: Payment — Stripe Checkout Redirect */}
          {step===5 && (
            <GlassCard style={{ padding:36 }}>
              <h3 style={{ fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:22, color:C.white, marginBottom:6 }}>Payment</h3>
              <p style={{ color:C.slate500, fontSize:13, marginBottom:24 }}>You&apos;ll be redirected to Stripe&apos;s secure checkout to complete payment.</p>

              {/* Order Summary */}
              <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:10, padding:20, marginBottom:24 }}>
                <div style={{ fontSize:11, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1.5, marginBottom:12 }}>Order Summary</div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:24 }}>
                ...
                <span style={{ color:C.slate500, fontSize:12 }}>You&apos;ll be redirected to Stripe&apos;s secure payment page. Your card details are never stored on our servers.</span>
              </div>

              {/* Stripe branding */}
              <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, marginBottom:28, padding:"16px 20px", borderRadius:10, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)" }}>
                <span style={{ fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:18, color:C.slate400, letterSpacing:1 }}>Powered by</span>
                <span style={{ fontFamily:"'Outfit',sans-serif", fontWeight:900, fontSize:22, background:"linear-gradient(135deg, #635BFF, #8B83FF)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>stripe</span>
              </div>

              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <Btn onClick={() => setStep(4)} variant="ghost">← Back</Btn>
                <Btn onClick={handleStripeCheckout} variant="gold" size="lg" disabled={paymentProcessing}>
                  {paymentProcessing ? "Redirecting to Stripe…" : `Pay ${totalEst} with Stripe →`}
                </Btn>
              </div>
            </GlassCard>
          )}
          ...
          {step===6 && (
            <GlassCard style={{ padding:64, textAlign:"center" }} glow={C.green+"40"}>
              ...
              <p style={{ color:C.slate500, fontSize:13, marginBottom:24 }}>SMS &amp; email confirmations sent. Payment processed securely via Stripe — receipt sent to your email.</p>
              <Btn onClick={() => { setStep(1); setSelDate(null); setSelTime(""); setSelPkg(""); setAddons([]); setWaiverChecked(false); setPaymentError(""); setPaymentPending(false); setForm({name:"",email:"",phone:"",vehicle:"",address:"",notes:""}); }} variant="ghost">Book Another Appointment</Btn>
            </GlassCard>
          )}
        </div>
      </div>
...
function AdminPage() {
  ...
  const statusColors: Record<string, string> = { confirmed:"#10B981", pending:"#F59E0B", completed:"#6366F1", cancelled:"#EF4444" };
        ...
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:16, marginBottom:36 }}>
            {[
              ...
              <GlassCard key={l as string} style={{ padding:20, textAlign:"center" }}>
                <div style={{ fontSize:24, marginBottom:6 }}>{ic as string}</div>
                <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:900, fontSize:28, color:c as string }}>{v as string | number}</div>
                <div style={{ color:C.slate500, fontSize:11, textTransform:"uppercase" as const, letterSpacing:1, marginTop:2 }}>{l as string}</div>
              </GlassCard>
            ))}
          </div>
                ...
                <thead>
                  <tr>
                    {["Ref","Customer","Service","Vehicle","Date & Time","Location","Total","Status"].map(h => (
                      <th key={h} style={{ padding:"12px 20px", textAlign:"left", fontSize:11, fontWeight:700, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1, borderBottom:"1px solid rgba(255,255,255,0.06)" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {mockBookings.map((b,i) => (
                    <tr key={b.id} style={{ background: i%2===0?"transparent":"rgba(255,255,255,0.015)", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                      ...
                      <td style={{ padding:"14px 20px" }}>
                        <span style={{ display:"inline-block", padding:"4px 12px", borderRadius:99, fontSize:11, fontWeight:700, textTransform:"uppercase" as const, letterSpacing:0.5, background:statusColors[b.status]+"22", color:statusColors[b.status] }}>
                          {b.status}
                        </span>
                      </td>
                    </tr>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16, marginTop:24 }}>
            {[
              ...
              <GlassCard key={t as string} style={{ padding:22, display:"flex", gap:14, alignItems:"center", cursor:"pointer" }}>
                <div style={{ fontSize:28 }}>{ic as string}</div>
                <div>
                  <div style={{ fontWeight:700, color:C.white, fontSize:14 }}>{t as string}</div>
                  <div style={{ color:C.slate500, fontSize:12, lineHeight:1.4 }}>{d as string}</div>
                </div>
              </GlassCard>
            ))}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1.4fr", gap:28 }}>
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              {[
                ...
                <GlassCard key={l as string} style={{ padding:"18px 22px", display:"flex", gap:14, alignItems:"center" }}>
                  <div style={{ fontSize:26 }}>{ic as string}</div>
                  <div>
                    <div style={{ fontSize:11, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1, marginBottom:2 }}>{l as string}</div>
                    {href ? <a href={href as string} style={{ color:C.blueLt, fontWeight:700, fontSize:15, textDecoration:"none" }}>{v as string}</a>
                      : <div style={{ color:C.white, fontWeight:600, fontSize:14 }}>{v as string}</div>}
                  </div>
                </GlassCard>
              ))}
              <GlassCard style={{ padding:"20px 22px" }}>
                <div style={{ fontSize:12, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1, marginBottom:10 }}>🕐 Availability</div>
              ...
              <GlassCard style={{ padding:"18px 22px" }}>
                <div style={{ fontSize:12, color:C.slate500, textTransform:"uppercase" as const, letterSpacing:1, marginBottom:10 }}>💳 Payment</div>
    ...
    <footer style={{ background:C.navyMid, borderTop:"1px solid rgba(255,255,255,0.07)", padding:"52px 28px 24px" }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr", gap:48, marginBottom:40 }}>
          ...
          <div>
            <h5 style={{ color:C.white, fontFamily:"'Outfit',sans-serif", fontSize:13, fontWeight:800, letterSpacing:1.5, textTransform:"uppercase" as const, marginBottom:14 }}>Quick Links</h5>
          ...
          <div>
            <h5 style={{ color:C.white, fontFamily:"'Outfit',sans-serif", fontSize:13, fontWeight:800, letterSpacing:1.5, textTransform:"uppercase" as const, marginBottom:14 }}>Packages</h5>
          ...
          <div>
            <h5 style={{ color:C.white, fontFamily:"'Outfit',sans-serif", fontSize:13, fontWeight:800, letterSpacing:1.5, textTransform:"uppercase" as const, marginBottom:14 }}>Contact</h5>
